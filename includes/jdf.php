 ...
 * 
 * استفاده:
 * jdate('Y/m/d', time()); // 1403/10/05
 * jdate('l، j F Y', time()); // یکشنبه، 5 دی 1403
 */

/**
 * تابع اصلی تبدیل تاریخ به شمسی
 */
function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa')
{
  ...
  $date = explode('_', date('H_i_s_j_n_Y_w_z_W', $ts));
  ...
  for ($i = 0; $i < $sl; $i++) {
    ...
    switch ($sub) {
      ...
      case 'c':
        $out .= $j_y . '/' . $j_m . '/' . $j_d . ' ،' . $date[0] . ':' . $date[1] . ':' . $date[2] . ' ' . (($date[0] < 12) ? 'ق.ظ' : 'ب.ظ');
        break;
      ...
      case 'o':
        ...
        $dny = 364 + $kab - $doy;
        $out .= ($doy < 8 and $jdw > 4) ? $j_y - 1 : (($dny < 8 and $jdw < 4) ? $j_y + 1 : $j_y);
        break;
...
/**
 * کلمات فارسی روز، ماه و ...
 */
function jdate_words($array, $type = '')
{
  foreach ($array as $type => $num) {
    ...
    switch ($type) {
      ...
      case 'rr':
        $key = array('یک', 'دو', 'سه', 'چهار', 'پنج', 'شش', 'هفت', 'هشت', 'نه', 'ده', 'یازده', 'دوازده');
        $array[$type] = $key[$num - 1];
        break;

      case 'rh':
        $key = array('یکشنبه', 'دوشنبه', 'سه‌شنبه', 'چهارشنبه', 'پنج‌شنبه', 'جمعه', 'شنبه');
        $array[$type] = $key[$num];
        break;
...
/**
 * تبدیل زمان Unix به timestamp قابل استفاده
 */
